...
def random_labelled_image(
    ...
    # ====== YOUR CODE: ======
    image = torch.randint(low=low, high = high, size=shape, generator=None, out=None, dtype=dtype, layout=torch.strided, device=None, requires_grad=False)
    label = torch.randint(low=0, high = num_classes, size=[1], generator=None, out=None, dtype=torch.int32).item()
...
def torch_temporary_seed(seed: int):
    ...
    # ====== YOUR CODE: ======
    temp=torch.random.get_rng_state()
    # ========================
    try:
        # ====== YOUR CODE: ======
        ...
        # ====== YOUR CODE: ======
        torch.random.set_rng_state(temp)
        # ========================
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        # ====== YOUR CODE: ======
        if index >= self.num_samples: # make sure that its actually suppose to be >= and not only >
            raise ValueError()
        with torch_temporary_seed(index):
            sample,label =random_labelled_image(shape=self.image_dim,num_classes=self.num_classes)
        return (sample,label)
        # ========================
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        # ====== YOUR CODE: ======
        while True:
            sample, labels = random_labelled_image(shape=self.image_dim, num_classes=self.num_classes)
            yield (sample, labels)
        return iter((sample,labels))
        # ========================
