...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            k_near_idx=torch.topk(input=dist_matrix.t()[i], k=self.k,largest=False).indices
            pred=self.y_train[k_near_idx]
            y_pred[i]=torch.mode(pred).values
            # ========================
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = None
    # ====== YOUR CODE: ======
    n_1, n_2 = x1.size(0), x2.size(0)
    norms_1 = torch.sum(x1 ** 2, dim=1, keepdim=True)
    norms_2 = torch.sum(x2 ** 2, dim=1, keepdim=True)
    norms = (norms_1.expand(n_1, n_2) +norms_2.transpose(0, 1).expand(n_1, n_2))
    dists = norms - 2 * x1.mm(x2.t())
    dists= torch.sqrt(torch.abs(dists))
    # ========================
...
def accuracy(y: Tensor, y_pred: Tensor):
    ...
    accuracy = None
    # ====== YOUR CODE: ======
    bool_list=torch.eq(y,y_pred).tolist()
    accuracy=(bool_list.count(True))/len(bool_list)
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for j in range(num_folds):
            ...
            temp_acc.append(accuracy(y_test,y_pred))
